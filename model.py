...
from keras.src.layers import Reshape, Conv2DTranspose, Conv2D
...
class EmbeddingSemvec(keras.Model):
    def __init__(self):
        ...
        initializer = tf.keras.initializers.TruncatedNormal(stddev=0.02, seed=1)
...
class LayoutNet(keras.Model):
    ...
    def generate(self, y, tr, ir, img, tex, z):
        ...
        G = self.generator(z, y=y_label, is_training=is_training)
    ...
    def call(self, x, y, tr, ir, img, tex, z, is_training=True):
        ...
        G = self.generator(z, is_training=is_training, y=y_label)

        # reconstruct result using the z calculated by encoder
        G_recon = self.generator(E, is_training=is_training, y=y_label)

        D_real = self.discriminator(x, is_training=is_training, y=ydis_label, z=z)
        D_fake = self.discriminator(G, is_training=is_training, y=ydis_label, z=z)
